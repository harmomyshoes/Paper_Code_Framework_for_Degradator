...
class MonteCarloOptimiser:
    ...
    def save_results(self, filefold, para_columns=[], is_outputfulldata = False):
        ...
        data_file_path = os.path.join(data_fold_path, f'MC_Data_BestResults_{datetime.now().strftime("%Y%m%d%H%M")}.csv')
        ...
        if is_outputfulldata and self._REINFORCE_logs is not None:
            ...
            MC_Data_Full_Path = os.path.join(data_fold_path, f'MC_Data_FullResults_{datetime.now().strftime("%Y%m%d%H%M")}.csv')
