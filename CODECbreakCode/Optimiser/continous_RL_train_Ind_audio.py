...
#cfg = get_config("FullTrack")
cfg = get_single_config()
train_cfg = cfg["training"]

class continous_RL_train_ind_audio(): 
    ...
    def train(self, update_num=0, eval_intv=0):
        ...
        for n in range(0,update_num):
            ...
            with tf.GradientTape() as tape:
                ...
                total = -J + train_cfg["param_alpha"]*regu_term + 0.001*ent.mean()

                elite_action = tf.stop_gradient(self._final_solution)        # shape [state_dim]
                mean_action = actions_distribution.mean()[0,0,:]             # [state_dim] (pick a canonical state, it's fine for now)

                bc_loss = tf.reduce_mean((mean_action - elite_action)**2)
                total += 0.1 * bc_loss
...
#Use the wrapper to create a TFEnvironments obj. (so that parallel computation is enabled)
    ...
    def save_results(self, filefold, para_columns=[], is_outputfulldata = False):
            ...
            data_file_path = os.path.join(data_fold_path, f'RL_Data_BestResults_{datetime.now().strftime("%Y%m%d%H%M")}.csv')
            ...
            if is_outputfulldata and self._REINFORCE_logs is not None:
                ...
                RL_Data_Full_Path = os.path.join(data_fold_path, f'RL_Data_FullResults_{datetime.now().strftime("%Y%m%d%H%M")}.csv')
