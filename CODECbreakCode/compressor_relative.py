...
class Compressor:
    """Compressor
    derived from compressor_qmul.py with changes to adapt to relative threshold compression
    ...
    def __init__(
        ...
        Args:
            ...
            relative_to_peak (bool): If True, threshold is relative to signal peak (e.g., -20 means 20 dB below peak).
                                 If False, threshold is absolute in dB.
        ...
        self.relative_to_peak = bool(relative_to_peak)  # Store the switch
    ...
    def __call__(
        ...
        Args:
            ...
            relative_threshold_db (float | None): If provided, use threshold 
        ...
        max_abs_amplitude = np.max(np.abs(input_signal))
        if max_abs_amplitude < self.eps:
            return input_signal.copy()
        peak_db = 20 * np.log10(max_abs_amplitude)

        # Calculate effective threshold
        if self.relative_to_peak:
            effective_threshold = peak_db + self.threshold  # threshold is relative
        else:
            effective_threshold = self.threshold  # threshold is absolute

        # Check if threshold is above signal peak - compressor would have no effect
        if effective_threshold >= peak_db:
            return input_signal.copy()

        # Compute the instantaneous desired levels
        input_signal_copy = input_signal.copy()  # Avoid modifying original
        input_signal_copy[input_signal_copy == 0] = self.eps
        x_g = 20 * np.log10(np.abs(input_signal_copy))
        x_g[x_g < -120] = -120
        y_g = effective_threshold + (x_g - effective_threshold) / self.ratio

        if self.knee_width == 0:
            # If knee width is zero, apply hard knee
            y_g[x_g < effective_threshold] = x_g[x_g < effective_threshold]
        else:
            # Apply soft knee
            index = 2 * np.abs(x_g - effective_threshold) <= self.knee_width
            y_g[index] = x_g[index] + (
                (1 / self.ratio - 1)
                * (x_g[index] - effective_threshold + self.knee_width / 2) ** 2
            ) / (2 * self.knee_width)

            index = 2 * (x_g - effective_threshold) < -self.knee_width
            y_g[index] = x_g[index]
