...
class GeneticOptimiser_Ind:
    ...
    def set_fitnessfun(self, fitness_fn: Callable[[np.ndarray], float], is_normalised: bool = False):
        # Pre-allocate array instead of using list
        total_evaluations = self._population_size * self._num_generations
        self._collect_data_list = np.zeros((total_evaluations, len(self._starting_population) + 1))
        self._current_index = 0
        
        def wrapped_reward_fn(ga_instance, solution, solution_idx):
            score = fitness_fn(solution, is_normalised)
            # Use array indexing instead of list append
            if self._current_index < len(self._collect_data_list):
                self._collect_data_list[self._current_index] = np.concatenate(([score], solution))
 ...
 #                           initial_population=preset_population,
    ...
    def save_results(self, filefold, para_columns=[], is_outputfulldata = True):
        ...
        data_file_path = os.path.join(data_fold_path, f'Evo_Data_BestResults_{datetime.now().strftime("%Y%m%d%H%M")}.csv')
        ...
        if is_outputfulldata:
            ...
            Evo_Data_Full_Path = os.path.join(data_fold_path, f'Evo_Data_FullResults_{datetime.now().strftime("%Y%m%d%H%M")}.csv')
