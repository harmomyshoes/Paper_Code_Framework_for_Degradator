...
from Optimiser.env_debug import Env_Continue as Env
...
#        self._REINFORCE_agent = None
    ...
    def train(self, update_num=0, eval_intv=0):
        ...
        for n in range(0,update_num):
            ...
            #     tf.summary.histogram("policy/mean", actions_distribution.mean(), step=n)
            #     tf.summary.histogram("policy/stddev", actions_distribution.stddev(), step=n)
            ...
            if n%eval_intv==0:
                ...
                ####print out the actions_distribution
    ...
    def set_agent(self):
        actor_net = actor_distribution_network.ActorDistributionNetwork(   
                                                ...
                                                continuous_projection_net=lambda spec: NormalProjectionNetwork(
                                                    spec,
                                                    state_dependent_std=True,     # <— HERE!in time of 17/7, in this way to let the prediction of seperate in different dimensions
#                                                    init_means_output_factor=1.0,       # ← mean-layer init scale, the factor will decide the resolution of the std exploration, by the equation:
                                                    # scale/fan_in. which is the number of the tensor number.
#                                                    std_bias_initializer_value=1.5
                                                    )     # ← log-std bias init, this value will decide the initial std exploration with the bias of the std layer
