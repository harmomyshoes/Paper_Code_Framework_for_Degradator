...
#        print(f"After LUFS, the mixing ouput in the RMS, Total: {round(NEUtil.calculate_rms_dB(mixing_data),2)}dB, Clipping Ratio&Cliped Num: {NEUtil.calcaulate_cliped_samples(mixing_data)}")
    ...
    def TestDynNoisedFullTrack(self,full_Manipul_list,outputfilename):
        ...
        #print(f"Before, The data shape is {mixing_data.shape}, the sample rate is {mixing_sr}")
        if mixing_data.ndim == 1:
            mixing_data = mixing_data.reshape(1, -1)
        #print(f"After, The data shape is {mixing_data.shape}, the sample rate is {mixing_sr}")


        mixing_data,mixing_sr = NoiseEffect.AddingHummingNoise_Single(mixing_data, mixing_sr,HumNoiseValue)
